...
export function PresentationWhatsAppFloat({ url }: PresentationWhatsAppFloatProps) {
  ...
  return (
    <a
      ...
      className={cn(
        'fixed right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full shadow-lg',
        'bottom-[calc(1.25rem+env(safe-area-inset-bottom,0px))]',
        'bg-[#25D366] text-white transition-transform hover:scale-105 hover:shadow-xl',
        'focus:outline-none focus:ring-2 focus:ring-[#25D366] focus:ring-offset-2',
        'sm:right-6 sm:bottom-[calc(1.5rem+env(safe-area-inset-bottom,0px))] sm:h-16 sm:w-16',
      )}
    >
